...
from ..utils import load_args, load_checkpoint, load_scalers, makedirs, timeit, fits_to_conds
...
def make_predictions(args: PredictArgs, smiles: List[List[str]] = None) -> List[List[Optional[float]]]:
    ...
    if args.dataset_type == 'multiclass':
        ...
        if args.outputmode=='arr':
            ...
            sum_preds = np.zeros((len(test_data), 4))
    ...
    if args.outputmode is not None:  
        conds = fits_to_conds(avg_preds,test_data.temps(),args.outputmode)
    ...
    elif args.outputmode=='arr':
        task_names.extend(['logA','Ea'])
    elif args.outputmode == 'slimnet':
        task_names.extend(['alpha', 'beta', 'gamma', 'theta'])
    elif args.outputmode=='vtf':
