...
from .data import StandardScaler, MoleculeDataset, preprocess_smiles_columns, get_task_names
...
def fits_to_conds(preds, temps, outputmode):
    """
    ...
    elif outputmode == 'slimnet':
        alpha = preds[:, 0]
        alpha = torch.FloatTensor(alpha).to(preds.device)
        beta = preds[:, 1]
        beta = torch.FloatTensor(beta).to(preds.device)
        gamma = preds[:, 2]
        gamma = torch.FloatTensor(gamma).to(preds.device)
        phi_theta = preds[:, 3]
