...
def train_SoftNet(model, epochs, train_loader, test_loader, X_test, y_test):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for i_train, data in enumerate(train_loader, 0):
            ...
            mid = torch.pow(gamma, beta).reshape(-1, 1)
            ...
            loss = p * loss_1 + 100*loss_2 + torch.pow(chain_order, 2)
            loss = torch.sum(loss)
            loss.backward()
            optimizer.step()
        ...
        # print(f'{epoch+1},{i+1} train_loss:{running_loss/(i_train+1)}, test_loss:{test_loss/(i_test+1)}')
            ...
            print('[%d] train_loss: %.5f, MAE: %.5f, R2: %.5f, ' % (epoch+1, running_loss/(i_train+1), MAE, R2))
        ...
        #print(best_valid)
            if (MAE) == best_valid:
                ...
                torch.save(checkpoint, os.path.join('softnet-test_y0.pt'))
...
if  __name__ == '__main__':
    ...
    train_SoftNet(model, 70000, train_loader, test_loader, X_test, y_test)