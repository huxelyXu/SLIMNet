...
def train_SLIMNet(model, epochs, train_loader, test_loader, X_test, y_test, name = 'softnet-test', out = 4, lam = 10):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for i_train, data in enumerate(train_loader, 0):
            ...
            inputs, labels = data
            optimizer.zero_grad()
            monomer_prop, scal, alpha, chain_order = model(inputs.to(device))
            beta = scal[:, :out]
            gamma = scal[:, out:]
            ...
            loss_1 = criterion(polymer_prop, labels.to(device))
            
            loss_2 = criterion(inputs[:, 168:174].to(device), monomer_prop)
        ...
        for i_test, data in enumerate(test_loader, 0):
            ...
            l1loss = nn.L1Loss()
            loss_1 = l1loss(polymer_prop, labels.to(device))
            
            R2 = mean_absolute_error(polymer_prop.detach().cpu().numpy(), labels.detach().cpu().numpy())
        ...
        # print(f'{epoch+1},{i+1} train_loss:{running_loss/(i_train+1)}, test_loss:{test_loss/(i_test+1)}')
            mmonomer_prop, scal, alpha, chain_order = model(torch.tensor(X_test).to(device).float())
            beta = scal[:, :out]
            gamma = scal[:, out:]
            mid = torch.pow(gamma, beta)
            prop_guass = torch.mul(alpha, mid)
            polymer_prop = chain_order + prop_guass
            y_predict_softnet = polymer_prop.detach().cpu().numpy()
            ...
            logging.info('[%d] train_loss: %.5f, MAE: %.5f, R2: %.5f, Best MAE: %.5f' % (epoch+1, running_loss/(i_train+1), MAE, R2, best_valid))
        ...
        #print(best_valid)
            if (MAE) == best_valid:
                ...
                torch.save(checkpoint, os.path.join(f'{name}_3.pt'))
...
if  __name__ == '__main__':
    ...
    for num in [1000, 900, 800, 750, 600, 500]:


        train_data, test_data = GetLoader(X_train[:num], y_train), GetLoader(X_test[:num], y_test)
        train_loader = torch.utils.data.DataLoader(train_data, batch_size=4, shuffle=True)
        test_loader = torch.utils.data.DataLoader(test_data, batch_size=100, shuffle=False)
    
        train_SLIMNet(model, 10000, train_loader, test_loader, X_test, y_test, name=f'softnet-test-{num}')