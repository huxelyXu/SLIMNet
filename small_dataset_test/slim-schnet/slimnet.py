...
def train_SoftNet(model, epochs, train_loader, test_loader, X_test, y_test, out = 1):
    criterion = nn.MSELoss()
    # criterion = nn.SmoothL1Loss()
    optimizer = optim.AdamW(model.parameters(), lr=1e-4, weight_decay=1e-5)
    scheduler = ReduceLROnPlateau(optimizer, factor=0.7, patience=700, verbose=True)
    # swa_model = AveragedModel(model)
    # scheduler = CosineAnnealingLR(optimizer, T_max=100)  # 使用学习率策略（余弦退火）
    # swa_start = epochs-10  # 设置SWA开始的周期，当epoch到该值的时候才开始记录模型的权重
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for i_train, data in enumerate(train_loader, 0):
            ...
            loss = 10000 * loss_1 + loss_2 + torch.pow(chain_order, 2)
            loss = torch.sum(loss)
            loss.backward()
            optimizer.step()
        ...
        #print(best_valid)
            ...
            # torch.save(swa_model.state_dict(),"swa_model.pt")
...
if  __name__ == '__main__':
    ...
    train_SoftNet(model, 500, train_loader, test_loader, X_test, y_test, out)